...
let JWTService = class JWTService {
    ...
    async verifyToken(token) {
        if (!token) {
            throw new rest_1.HttpErrors.Unauthorized("error verifying token");
        }
        let userProfile;
        try {
            const decryptedToken = await verifyAsync(token, this.jwtSecret);
            userProfile = Object.assign({ id: decryptedToken.id, name: decryptedToken.name });
        }
        catch (error) {
            throw new rest_1.HttpErrors.Unauthorized(`Error Veryfing Token ${error.message}`);
        }
        return userProfile;
    }
    async generateToken(userProfile) {
        if (!userProfile)
            throw new rest_1.HttpErrors.Unauthorized("Profile is empty");
        let token;
        try {
